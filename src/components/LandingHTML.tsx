...
import { RecentTrip as Trip } from '../store';

interface LandingHTMLProps {
  recentTrips: Trip[];
...
const LandingHTML = React.memo(({ 
  ...
  useEffect(() => {
    ...
    planningButtons.forEach(id => {
      const btn = containerRef.current?.querySelector(`#${id}`);
      if (btn) btn.addEventListener('click', handlePlanningClick);
    });

    pricingButtons.forEach(id => {
      const btn = containerRef.current?.querySelector(`#${id}`);
      if (btn) btn.addEventListener('click', handlePricingClick);
    });
    ...
    const logoLinks = containerRef.current?.querySelectorAll('.logo-link');
    ...
    // 原汁原味的多语言与动画数据
    const demoStates = Array.from(containerRef.current.querySelectorAll(".demo-state"));
    const demoDots = Array.from(containerRef.current.querySelectorAll(".demo-dot"));
    const demoTrack = containerRef.current.querySelector("#demo-track") as HTMLElement;
    const demoScrollThumb = containerRef.current.querySelector("#demo-scroll-thumb") as HTMLElement;
    const demoTitle = containerRef.current.querySelector("#demo-title");
    const demoIndex = containerRef.current.querySelector("#demo-index");
    const demoMiniLabel = containerRef.current.querySelector("#demo-mini-label");
    const demoCaption = containerRef.current.querySelector("#demo-caption");
    const demoProgress = containerRef.current.querySelector("#demo-progress") as HTMLElement;

    const translations = {
      en: {
        nav_features: "Features", nav_flow: "Flow", nav_archive: "Archive", nav_pricing: "Pricing", nav_contact: "Contact", nav_open_app: "Open App",
        hero_badge: "AI Travel Planner", hero_title: "Plan your trip<span class=\"block\">in one calm flow.</span>",
        ...
        hero_start: "Start Planning", hero_see_features: "See Features", hero_chip_destination: "Destination",
        hero_chip_interests: "Interests", hero_chip_time: "Time", hero_chip_itinerary: "Itinerary",
        preview_badge: "Product Preview", preview_copy: "The preview follows the same logic as the real app: set the trip, shape preferences, adjust timing, then refine the itinerary.",
        features_label: "Features", features_title: "A clearer product story, built around the real experience.",
        ...
        attraction_label: "Attraction Selection", attraction_title: "Freely choose the places you want in the trip.",
        ...
        selection_status_label: "Selection Status", selection_status_value: "2 attractions selected", selection_status_cta: "Generate itinerary",
        feature_card_1_title: "Trip setup", feature_card_1_copy: "Start with destinations, trip length, budget, dining style, transport preferences, and personal interests.",
        feature_card_2_title: "AI itinerary editing", feature_card_2_copy: "After generation, the workbench allows users to adjust timing, move activities, and request changes with AI assistance.",
        feature_card_3_title: "Booking and export", feature_card_3_copy: "When the plan is ready, users can continue into booking references and export a PDF version of the itinerary.",
        flow_label: "Flow", flow_title: "How the trip comes together inside WanderAI.",
        ...
        flow_card_1_title: "Input", flow_card_1_copy: "Users start by entering destinations, budget, duration, transport, and travel preferences.",
        flow_card_2_title: "Explore", flow_card_2_copy: "Recommended attractions are presented as cards so users can filter, compare, and freely select what they want.",
        flow_card_3_title: "Refine", flow_card_3_copy: "The itinerary workbench turns those selections into a schedule that can still be edited, reordered, or adjusted with AI.",
        flow_card_4_title: "Output", flow_card_4_copy: "Users continue to booking guidance and generate a PDF itinerary when the trip feels complete.",
        start_label: "Start", start_title: "Ready to explore the world?", start_copy: "Cleaner first impression, clearer product story, same real experience underneath.", start_cta: "Launch WanderAI",
        pricing_title: "Simple, transparent pricing", pricing_subtitle: "Start for free, upgrade when you need more.",
        ...
        archive_title: "Your Archives", archive_subtitle: "Pick up right where you left off.", archive_empty_title: "No trips planned yet", archive_empty_desc: "You haven't created any itineraries. Start planning your first adventure now!",
        archive_card_title: "Trip to Foshan, Guan...", archive_card_days: "3 Days", archive_card_date: "2026/3/28", archive_card_cta: "Continue Planning",
        footer_brand_tag: "AI Travel Planner", footer_brand_line_1: "Plan with less friction.", footer_brand_line_2: "Choose what matters.", footer_brand_line_3: "Refine until the trip feels right.",
        footer_product_heading: "Product", footer_product_link_1: "How It Works", footer_product_link_2: "Attraction Selection", footer_product_link_3: "Itinerary Editing", footer_product_link_4: "Open App",
        footer_contact_heading: "Contact", footer_bottom_copy: "© 2026 WanderAI. All rights reserved.", footer_bottom_location: "Made in Singapore",
      },
      zh: {
        nav_features: "功能", nav_flow: "流程", nav_archive: "存档", nav_pricing: "价格", nav_contact: "联系", nav_open_app: "打开应用",
        hero_badge: "AI 旅游规划助手", hero_title: "在宁静的流程中<span class=\"block\">规划您的旅程。</span>",
        hero_copy: "选择目的地，设置兴趣，调整时间，将其转化为您可以持续使用的真实行程。",
        hero_start: "开始规划", hero_see_features: "查看功能", hero_chip_destination: "目的地",
        hero_chip_interests: "兴趣", hero_chip_time: "时间", hero_chip_itinerary: "行程",
        preview_badge: "产品预览", preview_copy: "预览遵循与真实应用相同的逻辑：设置行程、塑造偏好、调整时间，然后完善行程。",
        features_label: "功能", features_title: "围绕真实体验构建的更清晰的产品故事。",
        features_copy: "WanderAI 不仅仅是生成一个结果。它让用户定义行程、浏览建议、自由选择景点，并在之后不断完善行程。",
        attraction_label: "景点选择", attraction_title: "自由选择您行程中想要的地点。",
        attraction_copy: "应用根据目的地和偏好推荐景点，但最终选择权仍属于用户。这让体验感觉是精心策划的，而非强加的。",
        attraction_bullet_1: "浏览带有类别、评分、简短描述和快速添加操作的推荐卡片。",
        attraction_bullet_2: "在决定哪些进入行程之前，按兴趣过滤并比较选项。",
        attraction_bullet_3: "仅在选择了足够多的景点后才生成行程，这保持了用户主导地位。",
        selection_status_label: "选择状态", selection_status_value: "已选择 2 个景点", selection_status_cta: "生成行程",
        feature_card_1_title: "行程设置", feature_card_1_copy: "从目的地、行程长度、预算、餐饮风格、交通偏好和个人兴趣开始。",
        feature_card_2_title: "AI 行程编辑", feature_card_2_copy: "生成后，工作台允许用户调整时间、移动活动，并在 AI 协助下请求更改。",
        feature_card_3_title: "预订与导出", feature_card_3_copy: "当计划就绪时，用户可以继续查看预订参考并导出 PDF 版本的行程。",
        flow_label: "流程", flow_title: "行程在 WanderAI 内部是如何汇聚而成的。",
        flow_copy: "首页保持简洁，但实际的产品流程更深。用户从设置到景点策划，再到行程完善和最终导出。",
        flow_card_1_title: "输入", flow_card_1_copy: "用户首先输入目的地、预算、时长、交通和旅行偏好。",
        flow_card_2_title: "探索", flow_card_2_copy: "推荐景点以卡片形式呈现，用户可以过滤、比较并自由选择他们想要的。",
        flow_card_3_title: "完善", flow_card_3_copy: "行程工作台将这些选择转化为一个时间表，仍然可以编辑、重新排序或使用 AI 调整。",
        flow_card_4_title: "输出", flow_card_4_copy: "用户继续查看预订指南，并在行程感觉完整时生成 PDF 行程。",
        start_label: "开始", start_title: "准备好探索世界了吗？", start_copy: "更简洁的第一印象，更清晰的产品故事，同样的真实体验。", start_cta: "启动 WanderAI",
        pricing_title: "简单透明的价格", pricing_subtitle: "免费开始，需要更多时再升级。",
        ...
        archive_title: "您的存档", archive_subtitle: "从您离开的地方继续。", archive_empty_title: "尚未规划行程", archive_empty_desc: "您还没有创建任何行程。现在就开始规划您的第一次冒险吧！",
        archive_card_title: "佛山、广州之旅...", archive_card_days: "3 天", archive_card_date: "2026/3/28", archive_card_cta: "继续规划",
        footer_brand_tag: "AI 旅游规划助手", footer_brand_line_1: "更少摩擦的规划。", footer_brand_line_2: "选择重要的部分。", footer_brand_line_3: "不断完善，直到行程感觉正确。",
        footer_product_heading: "产品", footer_product_link_1: "工作原理", footer_product_link_2: "景点选择", footer_product_link_3: "行程编辑", footer_product_link_4: "打开应用",
        footer_contact_heading: "联系", footer_bottom_copy: "© 2026 WanderAI。保留所有权利。", footer_bottom_location: "新加坡制造",
      }
    };

    const demoMeta = {
      en: [
        { title: "Choose destination", caption: "Start with city, budget, and trip style." },
        { title: "Select interests", caption: "Choose the interests that should shape the recommendations." },
        { title: "Adjust time", caption: "Tune duration before generating the plan." },
        { title: "Refine itinerary", caption: "Edit the schedule, then continue to booking links and PDF." },
      ],
      zh: [
        { title: "选择目的地", caption: "从城市、预算和旅行风格开始。" },
        { title: "选择兴趣", caption: "选择应塑造推荐的兴趣。" },
        { title: "调整时间", caption: "在生成计划前调整时长。" },
        { title: "完善行程", caption: "编辑时间表，然后继续查看预订链接和 PDF。" },
      ]
    };
    ...
    ctaButtons.forEach(function (id) {
      ...
      btn.addEventListener("click", function () {
        if (typeof (window as any).gtag === "function") {
          (window as any).gtag("event", "click_start_free_trial", { button_id: id, page_location: window.location.href });
        }
    ...
    function applyTranslations() {
      containerRef.current?.querySelectorAll("[data-i18n]").forEach(function (el) {
        const key = el.getAttribute("data-i18n");
        if (!key) return;
        el.textContent = (translations as any)[lang][key];
      });
      containerRef.current?.querySelectorAll("[data-i18n-html]").forEach(function (el) {
        const key = el.getAttribute("data-i18n-html");
        if (!key) return;
        el.innerHTML = (translations as any)[lang][key];
      });
      renderDemo(currentStepRef.current);
    }

    function renderDemo(step: number) {
      currentStepRef.current = step;
      demoStates.forEach(function (state, index) { state.classList.toggle("is-active", index === step); });
      ...
      const meta = (demoMeta as any)[lang][step];
      if (demoTitle) demoTitle.textContent = meta.title;
      if (demoCaption) demoCaption.textContent = meta.caption;
      if (demoMiniLabel) demoMiniLabel.textContent = "Step 0" + (step + 1);
      if (demoIndex) demoIndex.textContent = "0" + (step + 1) + "/04";
      if (demoProgress) demoProgress.style.width = ((step + 1) / (demoMeta as any)[lang].length) * 100 + "%";
      if (demoTrack) demoTrack.style.transform = "translate3d(0, -" + step * 25 + "%, 0)";
      if (demoScrollThumb) demoScrollThumb.style.transform = "translateY(" + step * 105 + "%)";
    }

    function startDemoLoop() {
      if (demoTimer) clearInterval(demoTimer);
      demoTimer = setInterval(function () {
        renderDemo((currentStepRef.current + 1) % (demoMeta as any)[lang].length);
      }, 2600);
    }
    ...
    const handleResize = () => renderDemo(currentStepRef.current);
    ...
    containerRef.current?.querySelectorAll(".section-fade").forEach(function (el) {
      observer.observe(el);
    });
    ...
    return () => {
      ...
      planningButtons.forEach(id => {
        const btn = containerRef.current?.querySelector(`#${id}`);
        if (btn) btn.removeEventListener('click', handlePlanningClick);
      });
      pricingButtons.forEach(id => {
        const btn = containerRef.current?.querySelector(`#${id}`);
        if (btn) btn.removeEventListener('click', handlePricingClick);
      });
      if (langToggle) {
  ...
  }, [recentTrips, onOpenLimitModal, onOpenWaitlistModal, onLoadTrip, onDeleteTrip, lang]);
  ...
  const archiveSectionHtml = `
      <section id="archive" class="scroll-mt-24 px-4 py-16 sm:px-6 lg:px-10 bg-slate-50/50">
        <div class="mx-auto max-w-7xl">
          <div class="section-fade text-center">
            <p class="text-xs font-bold uppercase tracking-[0.28em] text-slate-500" data-i18n="nav_archive">Archive</p>
            <h2 class="mx-auto mt-4 max-w-3xl text-4xl font-extrabold tracking-tight text-pine sm:text-5xl">
              <span data-i18n="archive_title">Your Archives</span>
            </h2>
            <p class="mx-auto mt-5 max-w-3xl text-lg leading-8 text-slate-600" data-i18n="archive_subtitle">
              Pick up right where you left off.
            </p>
          </div>
          
          <div class="section-fade mt-12">
            ...
            ` : `
              <div class="text-center py-12 bg-white rounded-3xl border border-slate-200 shadow-sm">
                <div class="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-4">
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-slate-400"><path d="M4 14.899A7 7 0 1 1 15.71 8h1.79a4.5 4.5 0 0 1 2.5 8.242"></path><path d="M12 12v9"></path><path d="m8 17 4 4 4-4"></path></svg>
                </div>
                <h3 class="text-lg font-semibold text-slate-900 mb-2" data-i18n="archive_empty_title">No trips planned yet</h3>
                <p class="text-slate-500 mb-6 max-w-md mx-auto" data-i18n="archive_empty_desc">You haven't created any itineraries. Start planning your first adventure now!</p>
                <button id="archive-start-btn" class="inline-flex items-center justify-center gap-2 rounded-full bg-[#047857] px-8 py-3.5 text-sm font-bold text-white shadow-lg shadow-[#047857]/20 transition-all hover:-translate-y-0.5 hover:bg-[#065f46] hover:shadow-xl">
  ...
  const pricingSectionHtml = `
      <section id="pricing" class="scroll-mt-24 px-4 py-16 sm:px-6 lg:px-10">
        <div class="mx-auto max-w-7xl">
          <div class="section-fade text-center">
            <p class="text-xs font-bold uppercase tracking-[0.28em] text-slate-500" data-i18n="nav_pricing">Pricing</p>
            <h2 class="mx-auto mt-4 max-w-3xl text-4xl font-extrabold tracking-tight text-pine sm:text-5xl" data-i18n="pricing_title">
              Simple, transparent pricing
            </h2>
            <p class="mx-auto mt-5 max-w-3xl text-lg leading-8 text-slate-600" data-i18n="pricing_subtitle">
              Start for free, upgrade when you need more.
            </p>
          </div>
          <div class="mt-12 grid grid-cols-1 gap-8 md:grid-cols-2 lg:max-w-4xl lg:mx-auto section-fade">
            <div class="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm flex flex-col">
              <h3 class="text-xl font-semibold text-slate-900" data-i18n="pricing_free_title">Free Trial</h3>
              <div class="mt-4 flex items-baseline text-5xl font-extrabold text-slate-900">
                <span data-i18n="pricing_free_price">$0</span>
                <span class="ml-1 text-xl font-medium text-slate-500" data-i18n="pricing_free_period">/forever</span>
              </div>
              <ul class="mt-8 space-y-4 flex-1">
                <li class="flex items-start">
                  ...
                  <span class="ml-3 text-slate-700"><span data-i18n="pricing_feature_itinerary">Number of Itinerary</span>: <strong data-i18n="pricing_free_itinerary">1</strong></span>
                </li>
                <li class="flex items-start">
                  <svg class="h-6 w-6 text-[#047857] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                  <span class="ml-3 text-slate-700"><span data-i18n="pricing_feature_iteration">Conversational Iteration</span>: <strong data-i18n="pricing_free_iteration">1 time</strong></span>
                </li>
                <li class="flex items-start">
                  <svg class="h-6 w-6 text-slate-300 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                  <span class="ml-3 text-slate-500"><span data-i18n="pricing_feature_pdf">Get PDF Itinerary</span>: <span data-i18n="pricing_free_pdf">No</span></span>
                </li>
                <li class="flex items-start">
                  <svg class="h-6 w-6 text-[#047857] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                  <span class="ml-3 text-slate-700"><span data-i18n="pricing_feature_preference">Preferences</span>: <strong data-i18n="pricing_free_preference">Basic 3 settings</strong></span>
                </li>
              </ul>
              <button id="pricing-free-btn" class="mt-8 block w-full rounded-full bg-slate-100 px-6 py-3 text-center text-sm font-semibold text-slate-900 hover:bg-slate-200 transition-colors" data-i18n="pricing_cta_free">Start Free Trial</button>
            </div>
  
            <div class="rounded-3xl border-2 border-[#047857] bg-white p-8 shadow-xl relative flex flex-col">
              <div class="absolute top-0 -translate-y-1/2 left-1/2 -translate-x-1/2">
                <span class="inline-flex rounded-full bg-[#047857] px-4 py-1 text-sm font-semibold tracking-wider text-white uppercase" data-i18n="pricing_pro_badge">Early Bird</span>
              </div>
              ...
              <ul class="mt-8 space-y-4 flex-1">
                <li class="flex items-start">
                  <svg class="h-6 w-6 text-[#047857] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                  <span class="ml-3 text-slate-700"><span data-i18n="pricing_feature_itinerary">Number of Itinerary</span>: <strong data-i18n="pricing_pro_itinerary">Unlimited</strong></span>
                </li>
                <li class="flex items-start">
                  <svg class="h-6 w-6 text-[#047857] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                  <span class="ml-3 text-slate-700"><span data-i18n="pricing_feature_iteration">Conversational Iteration</span>: <strong data-i18n="pricing_pro_iteration">Until you are satisfied</strong></span>
                </li>
                <li class="flex items-start">
                  <svg class="h-6 w-6 text-[#047857] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                  <span class="ml-3 text-slate-700"><span data-i18n="pricing_feature_pdf">Get PDF Itinerary</span>: <strong data-i18n="pricing_pro_pdf">Yes</strong></span>
                </li>
                <li class="flex items-start">
                  <svg class="h-6 w-6 text-[#047857] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                  <span class="ml-3 text-slate-700"><span data-i18n="pricing_feature_preference">Preferences</span>: <strong data-i18n="pricing_pro_preference">More than 20 options</strong></span>
                </li>
              </ul>
              <button id="pricing-pro-btn" class="mt-8 block w-full rounded-full bg-[#047857] px-6 py-3 text-center text-sm font-bold text-white hover:bg-[#065f46] transition-colors shadow-lg shadow-[#047857]/20" data-i18n="pricing_cta_pro">Get Pro</button>
            </div>
          </div>
        </div>
      </section>
  ...
  return (
    ...
    <nav class="fixed top-0 left-0 right-0 z-50 px-4 pt-5 sm:px-6 lg:px-10">
      <div class="glass mx-auto flex max-w-7xl items-center justify-between rounded-full border border-white/80 px-5 py-3 shadow-soft sm:px-7">
        <a href="#/" class="logo-link flex items-center gap-3 text-pine">
        ...
        <div class="hidden items-center gap-7 text-sm font-semibold text-slate-700 md:flex">
          ...
          <a href="#archive" class="transition-colors hover:text-pine" data-i18n="nav_archive">Archive</a>
          <a href="#pricing" class="transition-colors hover:text-pine" data-i18n="nav_pricing">Pricing</a>
        ...
        <div class="flex items-center gap-2">
          <button
            id="lang-toggle"
            class="rounded-full bg-white px-4 py-2 text-sm font-bold text-pine shadow-sm transition-all hover:bg-slate-50"
          >
            ${lang === 'en' ? '中文' : 'EN'}
          </button>
          <a
  ...
  return prevProps.recentTrips === nextProps.recentTrips;
});
